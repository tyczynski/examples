/// <reference types="react/experimental" />
import { useEffect, useState, experimental_useEffectEvent as useEffectEvent } from 'react'
import toast from 'react-hot-toast';

import { Layout } from '../ui/Layout';

export function UseEffectEventExample() {
  const [count, setCount] = useState(0)
  const [intervalValue, setIntervalValue] = useState(2000)
  
  const updateToast = useEffectEvent((currentCount) => {
    toast(`Count: ${currentCount}, Interval: ${intervalValue / 1000}s`);
  });

  useEffect(() => {
    toast('Updated effect');

    const intervalId = setInterval(() => updateToast(count), intervalValue)
    return () => clearInterval(intervalId)
    // eslint-disable-next-line react-hooks/exhaustive-deps -- useEffectEvent will not be taken into account by eslint-plugin-react-hooks
  }, [intervalValue])

  return (
    <Layout title="useEffectEvent">
      <button style={{marginBottom: 8 }} onClick={() => setCount((count) => count + 1)}>
        count is {count}
      </button>

      <br />

      <div>
        Interval: 
        {[1000, 2000, 3000].map((value) => (
          <span key={value}>
            <input type="radio" name="interval" value={value} checked={intervalValue === value} onChange={() => setIntervalValue(value)} />
            <label>{value / 1000}s</label>
          </span>
        ))}
      </div>
    </Layout>
  )
}
